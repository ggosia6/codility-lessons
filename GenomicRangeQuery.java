...
class Solution {
    ...
    public int[] solution(String S, int[] P, int[] Q) {
        ...
        for (int i = 0; i < P.length; i++) {
            ...
            int end = Q[i];
            if (S.length() == 1) {
                tab[i] = costs.get(S.charAt(0));
                continue;
            }
            if (S.length() == 2) {
                tab[i] = Math.min(costs.get(S.charAt(start)), costs.get(S.charAt(end)));
                continue;
            }
            if (start == end) {
                tab[i] = costs.get(S.charAt(start));
                continue;
            }
            if (patab[end + 1] - patab[start] > 0) {
                tab[i] = 1;
                continue;
            }
            if (pctab[end + 1] - pctab[start] > 0) {
                tab[i] = 2;
                continue;
            }
            if (pgtab[end + 1] - pgtab[start] > 0) {
                tab[i] = 3;
                continue;
            }
            if (pttab[end + 1] - pttab[start] > 0) {
                tab[i] = 4;
                continue;
            }
        }
